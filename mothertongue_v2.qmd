...
tongue = tongue.rename(columns = {"GEO" : "location", "REF_DATE" : "year"})
tongue = tongue[["year", "location", "Mother tongue", "Statistics", "VALUE"]]
tongue = pd.pivot_table(tongue, values = "VALUE", index = ["year", "location", "Mother tongue"], columns = "Statistics").reset_index()
```
...
```{python canada}
canada = tongue[(tongue["location"] == "Canada") & (tongue["Mother tongue"] != "Total, mother tongue")]

sb.set_style("darkgrid")

sb.lineplot(data = canada, x = "year", y = "Number",
hue = "Mother tongue").set_title("Number of speakers of mother tongues in Canada")
plt.show()

plt.clf()
sb.lineplot(data = canada, x = "year", y = "Percentage",
hue = "Mother tongue").set_title("Percentage of speakers of mother tongues in Canada")
plt.show()

# Suppress warning about chained indexing from Quarto Output
pd.options.mode.chained_assignment = None 

plt.clf()
canada["Percentage Change"] = canada.groupby("Mother tongue")["Percentage"].pct_change()
sb.lineplot(data = canada, x = "year", y = "Percentage Change",
hue = "Mother tongue").set_title("Percentage change over 4 years for different mother tongues in Canada")
plt.show()
```
