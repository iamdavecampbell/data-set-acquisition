...
data_download <- read.csv("https://files.ontario.ca/opendata/black_bear_2019.csv", stringsAsFactors = FALSE, header = TRUE)


total <- data_download %>% filter(WMU =="Total")
data <- data_download %>% filter(WMU !="Total")
...
### Total Number of Bear Hunters and Harvest

The following code plots  the total number of active hunters and bear harvested by year.

* The blue line shows the number of active black bear hunters in a year, its axis is on the left side. The purple line represents the number of black bear harvests in a given year and its axis is on the right side of the graph.
