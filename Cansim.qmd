...
```{python employmentprovincestimeseries}
...
g.fig.suptitle('Unemployment Rates by Province', fontsize=16)
plt.show()
...
plt.clf()
g = sb.FacetGrid(provinces[provinces["year"] < 2000],
                 col = "GEO",
                 col_wrap = 4,
                 height = 2)
g.map(sb.lineplot, "month", "VALUE", "year")
...
plt.clf()
plt.subplot(3,4,1)
sb.lineplot(data = provinces[provinces["GEO"] == "Alberta"],
            x = "month",
            y = "VALUE",
            hue = "year",
            palette = pal)
plt.subplot(3,4,2)            
sb.lineplot(data = provinces[provinces["GEO"] == "British Columbia"],
            x = "month",
            y = "VALUE",
            hue = "year",
            palette = pal)
...
test = dict(zip(range(1976, 2020), sb.color_palette("viridis")))
print(test)
