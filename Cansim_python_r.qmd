---
title: 'Accessing Statistics Canada Data Tables'
execute:
    # eval: false
...
categories: ["Census Data","Regression", "Time Series","Statistics Canada","Software:Python","Software:R", "Things that break", "API"]
summary: 'Using Statistics Canada Web Data Service endpoints to acquire socioeconomic data with Python and R.'
...
This showcases the _extended_ workflow, using the R `cansim` library and similarly finding the relevant files in python from the RESTapi.  Other pages in this repo do this in a less programmatic way, rather than querying the API to find the url for the zip file (in python), instead those pages suggest visiting the stat can page, finding the relevant table, copying the url and then downloading or (using a shortcut) assuming a known format for the url and plugging in the table of interest.

# Data Provider

Statistics Canada hosts a large set of socio-economic [datasets](https://www150.statcan.gc.ca/n1//en/type/data?MM=1#tables).  When searching through these, take note of the _table_ identifier.  Statistics Canada's [Web Data Service](https://www.statcan.gc.ca/en/developers/wds) provides official API endpoints for downloading tables and metadata.  In Python, we can use the WDS API directly with **requests** and **pandas**, rather than relying on a specialized wrapper package.  In R, the **cansim** library is a well maintained API wrapper for downloading these tables.  See also the [cansim](https://cran.r-project.org/web/packages/cansim/vignettes/cansim.html) vignette for examples and instructions to pull Statistics Canada datasets in R.
...
import requests # for calling Statistics Canada's Web Data Service
...
## Python
```{python}
# Download a full StatCan table ZIP once, cache it locally,
# and read the data CSV into pandas.

## query the API to obtain the location of the datafile.
...
# API set up
product_id = table_id.replace("-", "")[:8]
api_url = ("https://www150.statcan.gc.ca/t1/wds/rest/"
      f"getFullTableDownloadCSV/{product_id}/{language}")

# obtain the zipfile url from the api response
response = requests.get(api_url, timeout=30)
response.raise_for_status()
payload = response.json()
zip_url = payload["object"]

zip_path = cache_path / zip_url.split("/")[-1]
...
with ZipFile(zip_path) as zipped_table:
    csv_files = [
        name for name in zipped_table.namelist()
        if name.endswith(".csv") and "MetaData" not in name 
        ]
    with zipped_table.open(csv_files[0]) as file:
      table = pd.read_csv(file, low_memory=False)

table["REF_DATE"] = pd.to_datetime(table["REF_DATE"], errors="coerce")
data = table
...
provinces = provinces[(provinces["Gender"] == "Total - Gender") &
...
data |> select(GEO,Gender,Age.group,Labour.force.characteristics,REF_DATE,VALUE)|>
...
data |> select(GEO,Gender,Age.group,Labour.force.characteristics,REF_DATE,VALUE,Data.type)|>
  filter(Gender == "Total - Gender") |> 
  filter(GEO != "Canada")|> # since the Canada value will be a type of weighted average
  filter( Labour.force.characteristics == "Unemployment rate")|>
  filter(Age.group == "25 to 54 years")|>
...
data4plot = data |> select(GEO,Gender,Age.group,Labour.force.characteristics,REF_DATE,VALUE,Data.type)|>
  filter(Gender == "Total - Gender") |> 
  filter(GEO != "Canada")|> # since the Canada value will be a type of weighted average
  filter( Labour.force.characteristics == "Unemployment rate")|>
  filter(Age.group == "25 to 54 years")|>
...
unemployment = data |> select(GEO,Gender,Age.group,Labour.force.characteristics,REF_DATE,VALUE,Data.type)|>
  filter(Gender == "Total - Gender") |> 
  # filter(GEO != "Canada")|> # since the Canada value will be a type of weighted average
  filter( Labour.force.characteristics == "Unemployment rate")|>
  filter(Age.group == "25 to 54 years")|>
...
# extract the zipfile url from the api response
response = requests.get(api_url, timeout=30)
response.raise_for_status()
payload = response.json()
zip_url = payload["object"]
