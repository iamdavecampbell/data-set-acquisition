...
plants <- read.csv(unz(temp, "32100246.csv")) %>%as_tibble
unlink(temp) # Delete temp file

# Rename a couple of columns
plants <- plants %>% rename(year = REF_DATE, location = GEO)
...
```{r}
plants %>% subset(location == "Canada" & Output =="Production (number)") %>% select(year, Flowers.and.plants, VALUE)%>%
  spread(Flowers.and.plants,VALUE) %>%
  ggpairs()
