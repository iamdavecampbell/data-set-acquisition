...
## R
```{r rload daily data, eval=TRUE,message=FALSE}
# Find all csv files in the "Data" folder.
files2load = list.files(pattern="daily")
data_daily=NULL
for(lp in 1:length(files2load)){
  data_daily = rbind(data_daily,read_csv(paste0(files2load[lp])))
}
...
## Python
```{python pload ottawa monthly data}
...
## R
```{r rloadottawa monthly data, eval=TRUE}
# Find all csv files in the "Data" folder.
file2load = list.files(pattern="Ottawa-monthly")
data_monthly = read_csv(paste0(file2load))
